...
Pour un transfert de 10 MB représentant environ 6800 paquets (avec un MTU de 1500 bytes), même avec des buffers optimisés pour le calcul haute performance, les cycles répétés de remplissage et de vidage augmentent la probabilité de saturation momentanée. Ce phénomène est amplifié par le réseau partagé de Grid'5000, où le trafic concurrent (transferts NFS, connexions SSH, services de monitoring) peut créer des pics de congestion temporaires sur les switches communs.
...
**Fragmentation accrue** : Un message de 10 MB nécessite environ 6827 paquets (avec un MTU de 1500 bytes). Chaque paquet ajoute un overhead TCP/IP de 40 bytes, soit un total de 273 KB d'overhead (~2,6% du volume total), ce qui réduit le débit effectif.

**Garbage Collection Java** : Les gros transferts (> 5 MB) provoquent des allocations mémoire importantes et la génération d'objets temporaires lors de la sérialisation. Le garbage collector se déclenche alors pendant le transfert, créant des pauses de 5 à 50 millisecondes qui impactent directement le débit mesuré.
